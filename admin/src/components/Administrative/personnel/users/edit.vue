<template>
	<div class="m-l-50 m-t-30 w-500">
      ...
      <el-form-item label="岗位" prop="getAllselecct">
        <el-select  v-model="form.post_id" placeholder="请选择岗位" class="w-200">
          <el-option v-for="item in getAllselect" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getAllOrgs() {
        ...
        this.apiGet('admin/structures', data).then((res) => {
          this.handelResponse(res, (data) => {
      ...
      getAllselectw() {
        ...
        this.apiGet('admin/posts', data).then((res) => {
          this.handelResponse(res, (data) => {
            this.getAllselect = data
            // console.log(this.getAllselect)
          })
        })
      }
    },
